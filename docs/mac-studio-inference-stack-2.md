...
- **BWS project layout → two projects, split by CONSUMER not by subject matter (decided 2026-08-17).**
  - `homelab-infra` — read by the **control-node** machine account (token in the laptop Keychain, read-only). Proxmox credentials, network topology, SSH keys, k3s join tokens, the FRR password — and ESO's own access token.
  - an **apps** project — read by the **ESO** machine account. Application secrets only. Create it at the ESO milestone; an empty project just burns one of the free tier's three.
  - *One project for everything* rejected. A project is the access-control unit — machine accounts are granted per project — and the two consumers have very different exposure: the control-node token sits in a laptop Keychain, while **ESO's token lives in a Kubernetes Secret, readable by anything with cluster-admin or a pod exec in that namespace**. Sharing a project means a cluster compromise hands over the Proxmox API token, SSH keys and k3s join token — i.e. **cluster compromise escalates to hypervisor compromise**, via credentials ESO never needed. Two projects make that step impossible rather than merely unlikely.
  - ⚠ **`eso_bws_access_token` belongs in `homelab-infra`, not the apps project.** It is read by the control node (which seeds it into the cluster), and filing it with the app secrets would let ESO read and rotate the credential gating its own access. Same principle as secret zero: the thing that grants access cannot live behind the access it grants.
  - *Budget:* 3 projects and 3 machine accounts on the free tier. Infra + apps = 2 projects; control-node + ESO + the temporary migration-write account = 3 accounts, so **the write account must be deleted after the port** or ESO has no slot.
- **⚠ Cluster-bound does NOT mean ESO-managed — the BGP/topology values stay tier 2.** The natural assumption is that anything the cluster consumes should arrive via ESO. It cannot: ESO needs the Bitwarden SDK Server, which needs a cert-manager cert, which needs a Gateway, which needs a **LoadBalancer IP** — and the BGP config is what produces that IP. So `dmz_gateway`/`lb_range_base` reach the cluster as the **Ansible-seeded `cluster-topology` Secret**, and must keep doing so **permanently**, not just at first bootstrap: every rebuild needs BGP before ESO can exist. Moving `cluster-topology` to ESO "once things are up" would close a real circular dependency. (The two ASNs aren't secrets at all — cleartext constants in `group_vars/all/vars.yml`.)
...
**GitOps delivery → cosign-signed OCI artifact, consumed by a SYNC-LESS FluxInstance (decided 2026-08-29).**
- *Decision:* CI builds `gitops/` into an OCI artifact and **cosign-signs it keyless** via the GitHub Actions OIDC identity. The cluster consumes it through an `OCIRepository` carrying `spec.verify` + `matchOIDCIdentity`, so source-controller refuses to reconcile an unsigned or forged artifact. The `FluxInstance` is deliberately left **without `spec.sync`**; Ansible seeds the `OCIRepository` + root `Kustomization`, and those same manifests are committed **inside the path the root reconciles**, so Flux adopts them and thereafter drift-corrects them itself.
- *`spec.sync` (the flux-operator shortcut) rejected — it cannot express verification.* It accepts `OCIRepository` as a kind but exposes only 8 fields (`interval`, `kind`, `name`, `path`, `provider`, `pullSecret`, `ref`, `url`) and **no `verify`**. Confirmed against the live CRD *and* against the operator's `main` branch, where the `Sync` struct is unchanged and no issue tracks adding it — so this is a scope boundary, not a version gap. Using it would have looked finished while the signature gate was silently absent, which is the same class of failure as an unresolved `${var}` reconciling green.
- *Patching the generated source rejected:* the operator holds those objects in its `status.inventory` (34 entries) and reconciles them, so an in-place edit is reverted on the next pass.
- *GitHub artifact attestations rejected as the gate:* `OCIRepository.spec.verify.provider` is an enum of exactly `cosign`/`notation` — there is no attestation provider, so attestations **cannot** gate reconciliation. They compose fine alongside cosign as extra SLSA provenance for humans and `gh attestation verify`, but they are not admission control and must not be mistaken for it.
- *A long-lived cosign key rejected in favour of keyless:* a key is one more BWS secret to store, rotate and leak. Keyless + `matchOIDCIdentity` (issuer + subject) asserts *"signed by this workflow, in this repo"* — a stronger claim than *"signed by a key someone holds"* — with nothing at rest.
- *Does the operator still earn its place sync-less?* **Yes.** Sync is 2 of the 34 objects in its inventory. The other 32 are the whole Flux install (11 CRDs, 4 controller Deployments, RBAC, NetworkPolicies, ResourceQuota, namespace). It still pins and auto-upgrades the distribution within `2.9.x` (the k3s-sysupdate posture: patches unattended, minors deliberate), applies the `StrictPostBuildSubstitutions` patch via `spec.kustomize.patches`, and drift-corrects those 32 objects. We are declining the bootstrap shortcut, not the lifecycle manager.
- *Self-management is a layout property, not an object property* — confirmed in `flux2`'s `pkg/manifestgen/sync/sync.go`: `flux bootstrap` writes the source+root manifest to `<TargetPath>/<ns>/gotk-sync.yaml` while setting that Kustomization's `spec.path` to `./<TargetPath>`, i.e. the root reconciles a directory containing its own definition. We reproduce that deliberately; nothing about a root Kustomization self-heals unless you lay it out this way.
- *Consequences accepted:* (a) **pushing to `main` no longer reaches the cluster on its own** — only a signed artifact does, so the workflow becomes merge → CI signs → Flux picks it up; (b) the self-managed source **can disable its own verification** if a future signed artifact drops `verify` (not a hole — the artifact must already be signed by our workflow identity — but that one file warrants CI-secret-level scrutiny, not routine review); (c) a bad committed root is self-inflicted lockout, recovered by re-running `flux-bootstrap.yml`, which is why that play must stay idempotent and re-runnable rather than one-shot bootstrap.
